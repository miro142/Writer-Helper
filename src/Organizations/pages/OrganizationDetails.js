...
const OrganizationDetails =()=>{
    ...
    useEffect(()=>{
        const fetchOrganization = async () =>{
            try{
                const resData = await sendRequest(`http://localhost:5000/api/organizations/${organizationId}`);
    ...
    return  <React.Fragment>
        ...
        <div className="entity-item-dt_details"><Wrapper auth={auth} id={organizationId} items={loadedDetails} type="organization"/></div>
        </React.Fragment>
        }
  </React.Fragment>
}
